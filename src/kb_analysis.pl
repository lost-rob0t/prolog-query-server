...
positive_user_dependencies(References, Dependencies) :-
    findall(Signature,
            ( member(ref(_, Signature, positive, _), References),
              \+ builtin_signature_term(Signature, _)
            ),
            Raw),
...
fact_signature(Document, Signature) :-
    ...
    length(Args, Arity),
    normalize_name(Predicate, Name),
    Signature = sig(Name, Arity).
...
dependency_edges(References, Edges) :-
    findall(Edge,
            ( member(ref(From, To, Polarity, Source), References),
              \+ builtin_signature_term(To, _),
              signature_json(From, FromJSON),
              signature_json(To, ToJSON),
              atom_string(Polarity, PolarityString),
              Edge = _{from:FromJSON,
                       to:ToJSON,
...
arity_errors(Definitions, References, Errors) :-
    findall(use(Name, Arity, Source),
            user_signature_use(Definitions, References, Name, Arity, Source),
            Uses),
    findall(Name, member(use(Name, _, _), Uses), Names0),
    sort(Names0, Names),
    findall(Error,
            ( member(Name, Names),
              findall(Arity, member(use(Name, Arity, _), Uses), Arities0),
              sort(Arities0, Arities),
              Arities = [_,_|_],
              findall(Source, member(use(Name, _, Source), Uses), Sources0),
              sort(Sources0, Sources),
              Error = _{code:"arity_mismatch",
                        predicate:Name,
                        arities:Arities,
                        sources:Sources}
            ),
...
user_signature_use(Definitions, _References, Name, Arity, Source) :-
    member(def(sig(Name, Arity), Source, _), Definitions),
    \+ builtin_name_string(Name).
user_signature_use(_Definitions, References, Name, Arity, Source) :-
    member(ref(_, sig(Name, Arity), _, Source), References),
    \+ builtin_name_string(Name).
...
builtin_arity_errors(References, Errors) :-
    findall(Error,
            ( member(ref(_, sig(Name, Arity), _Polarity, Source), References),
              builtin_name_string(Name),
              atom_string(Predicate, Name),
              builtins:builtin_signature(Predicate, Expected),
              Arity =\= Expected,
              Error = _{code:"builtin_arity_mismatch",
                        predicate:Name,
                        expected:Expected,
                        actual:Arity,
                        source:Source}
...
undefined_warnings(Definitions, References, Warnings) :-
    findall(Warning,
            ( member(ref(_, Signature, positive, Source), References),
              ...
              Warning = _{code:"undefined_predicate",
                          predicate:SignatureJSON,
                          source:Source}
            ),
            Raw),
    sort(Raw, Warnings).
...
recursion_analysis(Definitions, References, Recursions, NegationErrors) :-
    ...
    findall(Recursion,
            ( member(Component, Components),
              recursive_component(Component, GraphEdges),
              ...
              Recursion = _{predicates:Predicates,
                            negative_cycle:NegativeCycle,
                            negative_edges:NegativeEdges}
            ),
            Recursions),
    findall(Error,
            ( member(Recursion, Recursions),
              get_dict(negative_cycle, Recursion, true),
              get_dict(predicates, Recursion, Predicates),
              get_dict(negative_edges, Recursion, NegativeEdges),
              Error = _{code:"non_stratified_negation",
...
graph_vertices(Definitions, References, Vertices) :-
    ...
    findall(S, (member(ref(_, S, _, _), References), \+ builtin_signature_term(S, _)), Refs),
    append(Defs, Refs, All),
    sort(All, Vertices).

user_graph_edges(References, Edges) :-
    findall(edge(From, To, Polarity, Source),
            (member(ref(From, To, Polarity, Source), References), \+ builtin_signature_term(To, _)),
            Edges).
...
strongly_connected_sets(Vertices, Edges, Components) :-
    findall(Component,
            ( member(Vertex, Vertices),
              findall(Other,
                      ( member(Other, Vertices),
                        mutually_reachable(Vertex, Other, Edges)
                      ),
                      Component0),
...
mutually_reachable(A, B, Edges) :-
    reachable(A, B, Edges),
    reachable(B, A, Edges).

reachable(From, To, Edges) :-
    reachable(From, To, Edges, [From]).

reachable(From, To, Edges, _Visited) :-
    member(edge(From, To, _, _), Edges).
reachable(From, To, Edges, Visited) :-
    member(edge(From, Next, _, _), Edges),
    \+ memberchk(Next, Visited),
    reachable(Next, To, Edges, [Next|Visited]).

recursive_component([Only], Edges) :-
    member(edge(Only, Only, _, _), Edges),
    !.
...
component_negative_edges(Component, Edges, JSONEdges) :-
    findall(JSON,
            ( member(edge(From, To, negative, Source), Edges),
              ...
              signature_json(From, FromJSON),
              signature_json(To, ToJSON),
              JSON = _{from:FromJSON, to:ToJSON, source:Source}
            ),
            JSONEdges).

reachability(Definitions, Rules, Reachable, UnreachablePredicates, UnreachableRules) :-
    findall(Signature, member(def(Signature, _, fact), Definitions), FactSeeds),
    findall(Head,
            (member(rule(_, Head, Dependencies), Rules), Dependencies == []),
            RuleSeeds),
    append(FactSeeds, RuleSeeds, Seeds0),
    sort(Seeds0, Seeds),
    reachability_fixpoint(Seeds, Rules, Reachable),
    findall(Signature, member(def(Signature, _, _), Definitions), Defined0),
    sort(Defined0, Defined),
    subtract(Defined, Reachable, UnreachablePredicates),
    findall(Source,
            ( member(rule(Source, _Head, Dependencies), Rules),
              \+ all_reachable(Dependencies, Reachable)
            ),
            UnreachableRules0),
    sort(UnreachableRules0, UnreachableRules).

reachability_fixpoint(Current, Rules, Reachable) :-
    findall(Head,
            ( member(rule(_, Head, Dependencies), Rules),
              all_reachable(Dependencies, Current)
            ),
            Derivable),
    append(Current, Derivable, Next0),
    sort(Next0, Next),
...
all_reachable([Dependency|Rest], Reachable) :-
    memberchk(Dependency, Reachable),
    all_reachable(Rest, Reachable).

unreachable_warnings([], [], []).
unreachable_warnings(Predicates, Rules, Warnings) :-
    findall(Warning,
            ( member(Signature, Predicates),
              signature_json(Signature, JSON),
              Warning = _{code:"unreachable_predicate", predicate:JSON}
            ),
            PredicateWarnings),
    findall(_{code:"unreachable_rule", source:Source}, member(Source, Rules), RuleWarnings),
    append(PredicateWarnings, RuleWarnings, Warnings).

definition_exists(Signature, Definitions) :-
    memberchk(def(Signature, _, _), Definitions).

builtin_signature_term(sig(Name, Arity), Expected) :-
    atom_string(Predicate, Name),
    builtins:builtin_signature(Predicate, Expected),
    Arity =:= Expected.
...
document_enabled(Document) :-
    (get_dict(enabled, Document, Enabled) -> Enabled \== false ; true).

document_source(Document, Source) :-
    (get_dict('_id', Document, Source0) -> Source = Source0 ; Source = "unsaved").

normalize_name(Value, Name) :-
    (string(Value) -> Name = Value ; atom(Value) -> atom_string(Value, Name)).
